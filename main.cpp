...
// using MacOS if you use another OS  you may change "/" to "\"
...
int main()
{
    ...
    for (int i = 0; i < ns; i++)
    {                         // calcola la differenza b - bc per il metodo adattivo (calculates b - bc for adaptive method)
    ...
    { // ciclo principale (main loop)
        ...
        for (int i = 0; i < ns; i++)
        {                                        // per ogni stadio (for each stage)
        ...
        if (Err <= toll)
        {                                    // se l'errore � minore della tolleranza (if the error is smaller than the tolerance)
            ...
            if (n == N)
            {            // se si � raggiunto il numero massimo di passi (if maximum number of steps is reached)
